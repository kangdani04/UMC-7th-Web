...
// Fetch 함수 타입 정의
const fetchMovieDetail = async (url: string): Promise<Movie> => {
...
const MovieDetail: React.FC<MovieDetailProps> = ({ url }) => {
  const {
    data: movie,
    isLoading,
    isError,
    ...
    queryKey: ["movieDetail", url], // Query Key
    queryFn: () => fetchMovieDetail(url), // Fetch Function
...
const BackgroundImage = styled.div<{ backdrop_path: string }>`
